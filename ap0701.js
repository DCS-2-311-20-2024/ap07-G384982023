...
function init() {
  ...
  const course0bject = new THREE.Line(
    new THREE.BufferGeometry().setFromPoints(points),
    new THREE.LineBasicMaterial({ color: "red"})
  );
  scene.add(course0bject);
  ...
  function render() {
    ...
    // コース表示の有無
    if (param.course) {
      course0bject.visible = true;
    }
    else{
      course0bject.visible = false;
    }
